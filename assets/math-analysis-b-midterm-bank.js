...
window.CaigouBankQuestions=[
  {
    ...
    "solution": {
      ...
      "method": "涉及收敛条件的判断，逐项检查量词和逆命题；一个具体反例就足以否定普遍断言。"
    }
  },
  {
    "no": 2,
    ...
    "solution": {
      "point": "无界与无穷大的区别",
      "lead": "无界只说明任意靠近 0 时仍能找到很大的函数值；无穷大则要求绝对值在足够靠近时始终大于任意给定正数。这里正弦既能取 1，也反复取 0，因此要用不同趋零数列分别检验。",
      "steps": [
        {
          "title": "构造趋向正无穷的数列",
      ...
      "method": "判断无界是否为无穷大，要检查函数值是否在足够小的去心邻域内始终足够大，而不只是偶尔出现大值。"
    }
  },
  {
    "no": 3,
    ...
    "solution": {
      ...
      "method": "三角函数的差先精确因式分解，再在乘积与商中使用等价无穷小，避免首项相消造成误判。"
    },
    "supplemental": true
  },
  {
    ...
    "solution": {
      ...
      "method": "间断点分类先求左右极限；内部表达式趋于无穷，不代表复合后的函数也趋于无穷。"
    }
  },
  {
    "no": 5,
    ...
    "solution": {
      ...
      "steps": [
        {
          "title": "由极限判断 b",
          "text": "先排除 \\(b=0\\)：分母为常数 \\(a+1\\)，若其为零则函数无定义，否则线性函数不趋于 0。若 \\(b>0\\)，当 \\(x\\to-\\infty\\) 时指数项趋于 0；无论 \\(a\\ne0\\) 还是 \\(a=0\\)，商均不可能趋于 0。因此只剩 \\(b<0\\) 需要检验。"
        },
        {
          "title": "核对所需极限",
      ...
      "method": "带参数函数的全局性质要同时满足极限条件与定义域条件，参数为零的边界情况也应单独检查。"
    }
  },
  {
    "no": 6,
    ...
    "solution": {
      ...
      "method": "用定义证明极限，可先把误差写成距离的倍数，再用固定邻域控制分母，最后反推允许的距离。"
    }
  },
  {
    "no": 7,
    ...
    "solution": {
      ...
      "method": "证明部分和收敛而不求具体极限时，可控制任意两项之差，用裂项求和验证柯西准则。"
    }
  },
  {
    "no": 8,
    ...
    "solution": {
      ...
      "method": "根式相减先有理化；复合函数求等价无穷小时，把内部变量完整代入，保留外面的乘因子。"
    }
  },
  {
    "no": 9,
    ...
    "solution": {
      ...
      "steps": [
        {
          "title": "叙述归结原则",
          "text": "设 \\(x_0\\) 是定义域的聚点。\\(\\lim_{x\\to x_0}f(x)=A\\) 当且仅当：对任意取自函数定义域且满足 \\(x_n\\ne x_0\\)、\\(x_n\\to x_0\\) 的数列，都有 \\(f(x_n)\\to A\\)。构造的趋近数列必须处处能代入原函数。"
        },
        {
          "title": "构造函数值为 0 的数列",
      ...
      "method": "证明函数极限不存在，可构造两条趋向同一点的合法数列，使函数值具有不同极限。"
    }
  },
  {
    "no": 10,
    ...
    "options": [],
    "answer": "存在 \\(x_n\\in S\\)，满足 \\(x_{n+1}>x_n\\) 且 \\(x_n\\to a\\)。",
    "solution": {
      ...
      "method": "构造趋近上确界的严格递增数列时，每一步同时约束“超过上一项”和“距离上界越来越小”。"
    }
  }
];
