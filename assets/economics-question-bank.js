(() => {
  ...
  document.body.innerHTML = `
    ...
    <dialog class="question-picker" id="question-picker" aria-labelledby="picker-title">
      <div class="picker-head"><div><b id="picker-title">跳转到任意题</b><span>已提交的题目会显示绿色</span></div><button type="button" class="picker-close" id="picker-close" aria-label="关闭题号选择">×</button></div>
  ...
  let state;
  try {
    state = { ...fresh, ...JSON.parse(localStorage.getItem(config.storageKey) || 'null') };
  } catch {
    state = fresh;
  }
  ...
  function syncPicker() {
    $('picker-grid').innerHTML = questions.map((q, index) => {
      ...
      return `<button type="button" class="${classes.join(' ')}" onclick="go(${index})"${index === state.current ? ' aria-current="true"' : ''}>${q.no}</button>`;
    }).join('');
  }
  ...
  function options(q) {
    ...
    if (!config.choiceMode) {
      return `<div class="options">${q.options.map((o, i) => `<div class="option">${String.fromCharCode(65 + i)}. ${esc(o)}</div>`).join('')}</div>`;
    }
    return `<div class="options choice-options">${q.options.map((o, i) => {
      const letter = String.fromCharCode(65 + i);
      const selected = state.chosen[q.no] === letter;
      return `<label class="option choice-option${selected ? ' selected' : ''}"><input type="radio" name="choice" value="${letter}"${selected ? ' checked' : ''}><span class="choice-letter">${letter}</span><span>${esc(o)}</span></label>`;
    }).join('')}</div>`;
  }
  ...
  function render(message = '') {
    ...
    const isChoice = Boolean(config.choiceMode && (q.options || []).length);
    const long = !['选择题', '填空题'].includes(q.type);
    const editor = isChoice
      ? '<div class="answer-label choice-prompt">先选择一个答案，再提交本题</div>'
      : `<label class="answer-label" for="draft">先写下你的答案或关键步骤</label><textarea class="draft${long ? '' : ' short'}" id="draft" placeholder="提交后才计入完成进度；解析默认隐藏。"></textarea>`;
    ...
    if (isChoice) {
      $('card').querySelectorAll('input[name="choice"]').forEach(input => input.addEventListener('change', event => {
        ...
        save();
        $('card').querySelectorAll('.choice-option').forEach(label => label.classList.toggle('selected', label.contains(event.target)));
      }));
    } else {
  ...
  function submitCurrent() {
    const q = questions[state.current];
    if (!(state.chosen[q.no] || '').trim()) {
      return alert(config.choiceMode && (q.options || []).length ? '请先选择一个答案。' : '先写下一点答案或思路吧。');
    }
    state.done[q.no] = true;
    save();
    render('已提交，本题计入完成进度；解析仍保持隐藏。');
  }

  function reveal() {
    const panel = $('solution');
    panel.classList.add('show');
    $('feedback').textContent = '参考解析已展开。';
    window.CaigouMath?.render(panel);
  }
